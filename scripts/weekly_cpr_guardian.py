...
def hlc_range(con,d1,d2):
    ...
    c=con.execute("SELECT spot_price FROM underlying_spot WHERE symbol='NIFTY' AND substr(snapshot_time,1,10) BETWEEN ? AND ? AND spot_price>0 ORDER BY snapshot_time DESC LIMIT 1",(d1,d2)).fetchone()
    return (r[0],r[1],c[0]) if r and r[0] and c else None
...
def assess(now_as_close):
    ...
    pw=hlc_range(con,(ws-timedelta(days=7)).isoformat(),(ws-timedelta(days=1)).isoformat())      # prior week
    ...
    pd=hlc_range(con,pdays[0],pdays[0]) if pdays else None                                        # prior day (Fri)
    ...
    W=cpr(*pw); D=cpr(*pd)
    ref=sp if (sp and now_as_close) else (sp or (op or pw[2]))
    o=op or ref; col="GREEN" if ref>=o else "RED"
    fri_close=pd[2]; gap=(o-fri_close)/fri_close*100 if fri_close else 0
    wpos=sd(ref,W["lo"],W["hi"]); dpos=sd(ref,D["lo"],D["hi"])
    agree=("AGREE-UP" if wpos==dpos=="ABOVE" else "AGREE-DOWN" if wpos==dpos=="BELOW"
           else "INSIDE" if (wpos=="INSIDE" and dpos=="INSIDE") else "DISAGREE")
    ...
    print(f"Pivots: S2 {W['s2']:.0f}  S1 {W['s1']:.0f}  PP {W['p']:.0f}  R1 {W['r1']:.0f}  R2 {W['r2']:.0f}")
...
def monitor(belo,behi):
    ...
    W=s["W"]; last=s.get("last",s["ref"]); cur=sd(sp,W["lo"],W["hi"]); flags=[]
    if s["wpos"] in ("ABOVE","BELOW") and cur!=s["wpos"]:
        flags.append(f"WEEKLY-CPR CROSS: was {s['wpos']} now {cur} (price {sp:.0f} vs band {W['lo']:.0f}-{W['hi']:.0f}) -> view may have flipped")
    if sp<=W["s1"]: flags.append(f"AT/BELOW S1 {W['s1']:.0f} (next S2 {W['s2']:.0f})")
    elif sp>=W["r1"]: flags.append(f"AT/ABOVE R1 {W['r1']:.0f} (next R2 {W['r2']:.0f})")
    if belo and sp<=belo*1.004: flags.append(f"BREAKEVEN THREAT (low): {sp:.0f} near BE {belo:.0f}")
    if behi and sp>=behi*0.996: flags.append(f"BREAKEVEN THREAT (high): {sp:.0f} near BE {behi:.0f}")
    mv=(sp-last)/last*100
    if abs(mv)>=0.4: flags.append(f"PRICE SPIKE {mv:+.2f}% ({sp-last:+.0f} pts) since last check")
    if ist().strftime("%H:%M")>="15:10": flags.append("DAY-CLOSE WINDOW (>=15:10) -> review PT/roll/exit")
    s["last"]=round(sp); save(s)
    if flags:
        print("ALERT "+ist().strftime("%H:%M"))
        for f in flags: print("  ! "+f)
        print(f"  ctx NIFTY {sp:.0f} | band {W['lo']:.0f}-{W['hi']:.0f} | S1 {W['s1']:.0f} R1 {W['r1']:.0f} S2 {W['s2']:.0f} R2 {W['r2']:.0f} | {s['agree']}")
    else:
        print(f"CALM {ist().strftime('%H:%M')} | NIFTY {sp:.0f} {cur} band | {s['agree']} intact | S1 {W['s1']:.0f} / R1 {W['r1']:.0f}")
