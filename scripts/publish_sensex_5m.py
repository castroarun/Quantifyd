"""Publish today's LIVE SENSEX 5-min OHLC to static/sensex_5m.json for the /app/nas
chart. Source = options_data.db underlying_spot (per-minute SENSEX spot, written
...
OUT = BASE / 'static' / 'sensex_5m.json'
...
rows = c.execute(
    ...
    "WHERE symbol='SENSEX' AND substr(snapshot_time,1,10)=? AND spot_price>0 "
...
def hlc(d1, d2=None):
    ...
    r = c.execute(
        ...
        "(SELECT spot_price FROM underlying_spot WHERE symbol='SENSEX' AND "
        " substr(snapshot_time,1,10)<=? AND spot_price>0 ORDER BY snapshot_time DESC LIMIT 1) "
        "FROM underlying_spot WHERE symbol='SENSEX' AND substr(snapshot_time,1,10) BETWEEN ? AND ? "
...
_all_days = [r[0] for r in c.execute(
    "SELECT DISTINCT substr(snapshot_time,1,10) d FROM underlying_spot WHERE symbol='SENSEX' ORDER BY d")]
