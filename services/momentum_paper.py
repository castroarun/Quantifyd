...
RULES = [
    ...
    ("Rebalance + buffer", "MONTHLY — last TRADING day of the month, ~14:45 IST",
     "Runs the Donchian exit check FIRST, then refills — so a name can no longer be bought at 14:45 and stopped out at 15:05. Re-ranks the 200 (the heavy step, run early to leave time to catch issues). Keeps a holding while it stays in the top 22; if it drops out, sells it and buys the best-ranked name not already owned."),
    ...
    ("Donchian stop", "DAILY — ~15:05 IST (before the 15:15-15:20 closing auction)",
     "If any holding is below its own prior-15-day low → exit just that one stock to cash. On month-end it also runs at ~14:45, ahead of the rebalance, so exits always precede buys. The 15:05 slot avoids the 15:15-15:20 closing auction, when NSE rejects new orders."),
...
def get_state():
    ...
    if cash > 1000:
        ...
        # TWO rows, not one. Previously a single row carried the ETF's unit count beside the TOTAL
        # idle cash, so 71 units of CASHIETF appeared to be worth Rs1,05,055 when they were worth
        # Rs76,508 — the rest being un-swept rupees. Splitting them makes each figure mean one thing.
        _u = _sweep_units()
        _since = (_get("sweep_since") or "") if _u else ""
        _swept_val = _sweep_value() if _u else 0.0
        ...
        if _u:
            holdings.insert(0, dict(
                ...
                price=round(_swept_val / _u, 2) if _u else None,
                value=round(_swept_val),
                weight=round(_swept_val / nav * 100, 1) if nav else 0,
