"""
NAS — Nifty ATR Strangle — Database Layer
============================================
SQLite persistence for intraday Nifty options strangle system.
Pattern follows BnfDB / MaruthiDB exactly.

Tables:
  - nas_state: single-row system state
  - nas_positions: active/closed option positions
  - nas_trades: completed trades with P&L
  - nas_orders: order audit log
  - nas_signals: signal event history
  - nas_daily_state: EOD snapshots for equity curve
  - nas_option_snapshots: periodic option chain captures
"""
...
DB_PATH = str(DATA_DIR / "nas_trading.db")


class NasDB:
    """SQLite persistence for NAS intraday strangle trading system."""
        ...
        with self.db_lock:
            ...
            try:
                conn.executescript("""
                    ...
                    INSERT OR IGNORE INTO nas_state (id) VALUES (1);

                    -- Active and closed positions
                    CREATE TABLE IF NOT EXISTS nas_positions (
                    ...
                    CREATE INDEX IF NOT EXISTS idx_nas_pos_status
                        ON nas_positions(status);
                    CREATE INDEX IF NOT EXISTS idx_nas_pos_strangle
                        ON nas_positions(strangle_id, status);

                    -- Completed trades (closed position pairs)
                    CREATE TABLE IF NOT EXISTS nas_trades (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        strangle_id INTEGER,
                    ...
                    CREATE INDEX IF NOT EXISTS idx_nas_trades_date
                        ON nas_trades(trade_date);

                    -- Order audit log
                    CREATE TABLE IF NOT EXISTS nas_orders (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                    ...
                    CREATE TABLE IF NOT EXISTS nas_signals (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                    ...
                    CREATE TABLE IF NOT EXISTS nas_daily_state (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                    ...
                    -- Option chain snapshots (periodic captures)
                    CREATE TABLE IF NOT EXISTS nas_option_snapshots (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    def get_state(self):
        with self.db_lock:
            conn = self._get_conn()
            try:
                row = conn.execute("SELECT * FROM nas_state WHERE id=1").fetchone()
    ...
    def update_state(self, **kwargs):
        ...
        with self.db_lock:
            conn = self._get_conn()
            try:
                conn.execute(f"UPDATE nas_state SET {cols} WHERE id=1", vals)
                conn.commit()
            finally:
                conn.close()
    ...
    def add_position(self, **kwargs):
        ...
        with self.db_lock:
            ...
            try:
                cur = conn.execute(
                    f"INSERT INTO nas_positions ({cols}) VALUES ({placeholders})",
                    list(kwargs.values()))
                conn.commit()
    ...
    def get_active_positions(self, leg=None, strangle_id=None):
        with self.db_lock:
            conn = self._get_conn()
            try:
                sql = "SELECT * FROM nas_positions WHERE status='ACTIVE'"
                params = []
                if leg:
                    sql += " AND leg=?"
                    params.append(leg)
    ...
    def get_today_closed_positions(self):
        ...
        with self.db_lock:
            ...
            try:
                rows = conn.execute(
                    "SELECT * FROM nas_positions WHERE status='CLOSED' AND created_at >= ? ORDER BY id",
                    (today,)
                ).fetchall()
                return [dict(r) for r in rows]
            finally:
    ...
    def update_position(self, position_id, **kwargs):
        ...
        with self.db_lock:
            conn = self._get_conn()
            try:
                conn.execute(f"UPDATE nas_positions SET {cols} WHERE id=?", vals)
                conn.commit()
            finally:
                conn.close()
    ...
    def get_next_strangle_id(self):
        with self.db_lock:
            ...
            try:
                row = conn.execute(
                    "SELECT COALESCE(MAX(strangle_id), 0) + 1 AS next_id FROM nas_positions"
                ).fetchone()
                return row['next_id']
            finally:
                conn.close()
    ...
    def log_order(self, **kwargs):
        ...
        with self.db_lock:
            ...
            try:
                cur = conn.execute(
                    f"INSERT INTO nas_orders ({cols}) VALUES ({placeholders})",
                    list(kwargs.values()))
                conn.commit()
                return cur.lastrowid
            finally:
    ...
    def get_today_order_count(self):
        ...
        with self.db_lock:
            ...
            try:
                row = conn.execute(
                    "SELECT COUNT(*) as cnt FROM nas_orders WHERE created_at >= ?",
                    (today,)).fetchone()
                return row['cnt']
            finally:
                conn.close()
    ...
    def log_signal(self, **kwargs):
        ...
        with self.db_lock:
            ...
            try:
                conn.execute(
                    f"INSERT INTO nas_signals ({cols}) VALUES ({placeholders})",
                    list(kwargs.values()))
                conn.commit()
            finally:
                conn.close()
    ...
    def get_recent_signals(self, limit=20):
        with self.db_lock:
            ...
            try:
                rows = conn.execute(
                    "SELECT * FROM nas_signals ORDER BY created_at DESC LIMIT ?",
                    (limit,)).fetchall()
                return [dict(r) for r in rows]
            finally:
                conn.close()
    ...
    def add_trade(self, **kwargs):
        ...
        with self.db_lock:
            ...
            try:
                cur = conn.execute(
                    f"INSERT INTO nas_trades ({cols}) VALUES ({placeholders})",
                    list(kwargs.values()))
                conn.commit()
                return cur.lastrowid
            finally:
    ...
    def get_recent_trades(self, limit=50):
        with self.db_lock:
            ...
            try:
                rows = conn.execute(
                    "SELECT * FROM nas_trades ORDER BY created_at DESC LIMIT ?",
                    (limit,)).fetchall()
                return [dict(r) for r in rows]
            finally:
                conn.close()
    ...
    def get_stats(self):
        with self.db_lock:
            conn = self._get_conn()
            try:
                trades = conn.execute("SELECT * FROM nas_trades").fetchall()
                if not trades:
                    return {
    ...
    def save_daily_state(self, **kwargs):
        ...
        with self.db_lock:
            ...
            try:
                conn.execute(f"""
                    INSERT INTO nas_daily_state ({cols}) VALUES ({placeholders})
    ...
    def get_equity_curve(self):
        with self.db_lock:
            ...
            try:
                rows = conn.execute(
                    "SELECT * FROM nas_daily_state ORDER BY trade_date"
                ).fetchall()
                return [dict(r) for r in rows]
            finally:
                conn.close()
    ...
    def save_option_snapshot(self, snapshot_time, options_data):
        """Save a batch of option chain data points."""
        with self.db_lock:
            conn = self._get_conn()
            try:
                for opt in options_data:
                    conn.execute("""
                        INSERT INTO nas_option_snapshots
                        (snapshot_time, expiry_date, strike, instrument_type,
    ...
    def get_recent_snapshots(self, limit=100):
        with self.db_lock:
            ...
            try:
                rows = conn.execute(
                    "SELECT * FROM nas_option_snapshots ORDER BY snapshot_time DESC LIMIT ?",
                    (limit,)).fetchall()
                return [dict(r) for r in rows]
            finally:
                conn.close()
