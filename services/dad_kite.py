"""Dad's (secondary) Zerodha account — READ-ONLY Kite client, fully isolated from the
primary kite_service so it can never affect the live trading account.

Separate api_key/secret + a separate token file (backtest_data/dad_access_token.json),
refreshed daily by scripts/dad_auto_login.py (TOTP). The client is order-BLOCKED:
place/modify/cancel raise — this account is for monitoring holdings only.
...
def get_dad_kite() -> KiteConnect:
    """READ-ONLY KiteConnect for Dad's account (orders blocked)."""
