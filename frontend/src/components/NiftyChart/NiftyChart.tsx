...
// (TradingView's free embed is delayed for NSE, so we render our own.)

interface Candle { t: string; o: number; h: number; l: number; c: number; }
interface Data { updated: string; day: string; last: number | null; candles: Candle[]; }
...
export default function NiftyChart() {
  ...
  useEffect(() => {
    ...
    const load = () =>
      fetch(`/api/nas/ticker/status`, { cache: 'no-store' })
        .then((r) => r.json())
        .then((d) => { if (on && d.last_ltp) setLtp(d.last_ltp); })
        .catch(() => {});
    load();
  ...
  const slot = plotW / n;
  const bodyW = Math.max(2, Math.min(slot * 0.62, 13));
  ...
  return (
    <section style={{ margin: '0 0 18px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, margin: '0 0 8px', flexWrap: 'wrap' }}>
        ...
        {cur != null && (
          <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--accent,#f5b301)' }}>{cur.toFixed(2)}</span>
        )}
        <span style={{ fontSize: 11, color: 'var(--ink-muted,#8b93a1)' }}>
          live {ltp ? '●' : '○'} · candles {data?.updated ?? '–'}
        </span>
      </div>
      <div
        ref={wrapRef}
        style={{
          height: H, width: '100%', background: 'var(--surface,#0f131a)',
          border: '1px solid var(--border,#232936)', borderRadius: 12, overflow: 'hidden',
        }}
      >
        {candles.length === 0 ? (
