...
export const SYSTEMS: StrategySystem[] = [
  ...
  },
  {
    ...
    changeLog: [
      { date: '13 Sep 2026', text: 'Fixed: the weekly gate check now runs on the last TRADING day of the week. It used to skip weekends but not NSE holidays, so in a week whose Friday is a holiday (2-Oct and 25-Dec-2026) it ran on the holiday itself, where every order is refused, and that week\u2019s gate action slipped a full week. The 15:05 end-of-day job now also skips NSE holidays. No rule changed.' },
  ...
  },
  {
    ...
    since: '2 Sep 2026',
    rule: 'LIVE from 13-Sep-2026 (flipped Sunday 13:33 IST; first evening Mon 14-Sep 18:50). First close above an all-time-high close that is at least 60 trading bars old and at least 20% above the low in between; 20-day traded value \u2265 \u20b92 cr; bought at the NEXT open. Exit: SuperTrend(14,4) on the close, sold at the next open. No stop, no time stop. 16 slots at 6.25% of NAV; contested slots go to the most liquid name. Swap rule OA-ROT-1 live under the same switch.',
    rules: [
      ['Base Age signal (staged)', 'Close > prior ATH close; that prior high \u2265 60 bars old; close fell \u2265 20% below it in between; 60-bar per-symbol re-arm; tv20 \u2265 \u20b92 cr; funds excluded by NAME (etf_exclusions.json). Replication gate vs research/164: 562/562 exact.'],
      ['Base Age exit (staged)', 'SuperTrend(14,4) direction on the official close, confirmed after the evening data refresh, sold at the NEXT open. No \u22128% stop, no 15-SMA trail. Dry-run 11-Sep-2026: all 11 live positions HOLD, 13-26% above the line.'],
      ['Base Age book (staged)', '16 slots at 6.25% of NAV; buy only if cash \u2265 the slot (else logged refused-for-cash); contested slots to the largest 20-day traded value (research/164, deviation D1 from the study random draw); AMO MARKET buy at the next open, AMO LIMIT at close \u00b12% fallback.'],
      ['Swap rule OA-ROT-1 (staged)', 'On an evening when a qualifying signal is refused (no free slot, or cash below one slot): sell the holding with the largest loss versus its buy price IF that loss is worse than \u221210% on the trigger close, and buy the refused signal with the highest 12-month relative strength (rs252, on the study\u2019s NIFTYBEES master calendar). Both legs AMO for the next open, SELL first, tagged OA-ROT1-SELL / OA-ROT1-BUY. At most ONE swap per evening; a holding already being sold on the trail is an ordinary exit, not a swap; if no refused entrant can be funded from the proceeds, both legs are cancelled. research/170 Part B: 22.58% after tax / \u221231.78% DD / Calmar 0.710 against the un-rotated 20.95 / \u221234.05 / 0.611 \u2014 +0.105 paired Calmar on 30/30 fresh seeds, +1.65pp CAGR on 60/60 pooled paths, beating a rate-matched random swap 60/60. ADOPTED by Arun 13-Sep-2026 over research/170\u2019s own non-adoption verdict (it missed the pre-registered +0.10 Calmar bar by 0.004). Replication gate: 8,488 of 8,488 rotation decisions identical to research/170\u2019s engine \u2014 fire, name sold and name bought. Own OFF switch: OA_ROT1 in services/oa_real.py disables the swap and keeps Base Age.'],
      ['Switch', 'OA_RULESET in services/oa_real.py: legacy (running) | baseage (staged), and OA_ROT1: True | False, which disables the OA-ROT-1 swap alone while keeping Base Age. Entries paused 11-Sep-2026; Base Age entries staged 12-Sep-2026 and the swap rule 13-Sep-2026, switch OFF. BLOCKER before the flip: the 09:20 equity_executor OA top-up job must be pointed at the IPO book.'],
  ...
  },
  {
    ...
    dashboard: '/ipo-paper',
    studies: [
      { slug: "ipo-rules-universe-transplant-research169", title: "Why is it IPO-specific? The rules on Nifty 50 → all stocks fail everywhere — and the live MIN_BARS 60 is not the validated spec", verdict: 'NO EDGE' },
    ...
    changeLog: [
      { date: '13 Sep 2026', text: "Automated: exits are now PLACED by the book. Before, a stop, target or trail booked the sale at the signal close and only alerted \u2018Place it\u2019 \u2014 no order, and the reconcile never looked at sells, so a missed sale left the name unstopped while the book counted its cash as free. Now the 18:45 run places a next-open SELL tagged IPO-EXIT (MARKET, LIMIT 2% under the close as fallback, retried the next evening if refused) and the sale is booked only when the broker fills it, at the broker\u2019s price. Deviation being measured: the backtest sells at the signal close." },
...
note: 'EXECUTOR DEPLOYED, NOT ARMED. The cron runs unarmed (dry-run: it logs the exact orders and sends nothing) until backtest_data/straddle45_ARMED exists. Next entry 11 Sep 2026 for the 27 Oct expiry \u2014 which on current volatility is a SKIP: India VIX 11.15, rank 17.9, filter needs > 25. The order-placement path itself has never been exercised; a first-fire review is registered for 12 Sep 2026.',
  },
  {
    id: 'oa-v2',
    name: 'OA V2.0',
    subtitle: 'NSE cash \u2014 new all-time-high close out of an AGED, DEEP base (research/161)',
    status: 'parked',
    size: 'none \u2014 research only, no capital, no paper book',
    since: '11 Sep 2026',
    rule: 'CANDIDATE \u2014 RESEARCH ONLY, nothing deployed. A close above the prior all-time-high close, where that prior high is at least 60 trading bars old AND price fell at least 20% below it in between \u2192 buy at the NEXT OPEN; exit on a SuperTrend(14,4) close trail, no hard stop; 16 slots at 6.25% of NAV. No volume filter (tested and rejected), no market gate, no RS filter (untested).',
    rules: [
      ['Universe', 'All NSE dailies, 20d-median traded value \u2265 \u20b92 cr, ETFs excluded. At a \u20b95 cr floor it returns 20.62% instead of 21.26%.'],
      ['Signal', 'close > prior all-time-high close, where the ATH is the running maximum of closes STRICTLY BEFORE that day'],
      ['Base age (the new rule)', 'that prior all-time high must be at least 60 trading bars old. X=40 is statistically identical (21.23% vs 21.26%), so the winner sits on a plateau.'],
      ['Base depth (the new rule)', 'price must have fallen at least 20% below the prior high somewhere in that gap. Age WITHOUT depth is a shallow pause and does not pay.'],
      ['Volume', 'NONE. Tested at 2x / 3x / 5x the prior 20-bar median: it raises expectancy per trade (+12.4% \u2192 +15.1%) and makes the BOOK worse (CAGR 21.26 \u2192 20.51%, drawdown \u221234.8 \u2192 \u221244.1%) because a 5x filter discards 60% of the events a 16-slot book needs.'],
      ['Entry fill', 'NEXT-DAY OPEN. Honest on both legs \u2014 nothing is filled on the bar that generated the signal. This is the discipline Open Alpha\u2019s published backtest lacked.'],
      ['Exit', 'SuperTrend(14,4) on the close, no hard stop. This single swap is worth +11.85pp over Open Alpha\u2019s 15-SMA trail + \u22128% stop on the SAME entries, and is the largest effect in the study.'],
      ['Book', '16 slots at 6.25% of NAV, \u20b910L, 25 bps/side, idle cash 5.5%, after tax (20% STCG / 12.5% LTCG, Indian FY loss netting)'],
      ['Split guard', 'market_data.db is not retroactively split adjusted, so the ATH is computed only from bars after the last day-over-day fall worse than \u221235%'],
      ['Measured (research/161)', '21.26% CAGR after tax [30-seed band 19.87\u201321.89] at \u221234.80% drawdown, Calmar 0.618, win rate 49.2%, expectancy +12.43%/trade, ~32 trades/yr, max losing streak 14. Pre-2016 19.33% / \u221232.45%; 2016+ 23.24% / \u221232.43%. NIFTYBEES over the same window: 12.30% / \u221259.71%.'],
      ['NOT YET DONE', 'RS\u226570 was never tested here; the portfolio-fit test against Open Alpha cannot run until OA is honestly re-measured; no paper book has been started and no order has ever been placed on this rule set.'],
    ],
    rulesDoc: 'research/161_ath_base_age_breakout/ATH_BASE_AGE_VOLUME_BREAKOUT_DAILY_SWEEP_STATUS.md',
    studies: [{ slug: 'ath-base-age-breakout-research161', title: 'OA V2.0 \u2014 new-ATH breakout after an aged, deep base (research/161)', verdict: 'STRATEGY-CANDIDATE' }],
    changeLog: [
      { date: '11 Sep 2026', text: 'NAMED "OA V2.0" by Arun after research/161. NOTHING DEPLOYED \u2014 no capital, no paper book, no executor, no crontab entry. The study cleared all five pre-registered criteria (\u226520% after-tax CAGR on the 30-seed median, beating NIFTYBEES on return AND drawdown in BOTH windows, beating an honest in-engine Open Alpha proxy by 14.45pp and a date-matched random control by 9.15pp, on a plateau) but three things are outstanding before it could be considered: the worst of 30 seeds is 19.87%, a tenth of a point under the floor; removing the ten best of 687 trades collapses compounding by 133,000x, so it is still tail-carried; and the portfolio-fit test against Open Alpha is blocked until that book has been honestly re-measured (its published 34.9% rests on a same-bar look-ahead fill).' },
    ],
    note: 'Research candidate only. The headline finding is about the EXIT, not the entry: the same plain all-time-high entries return 6.81% with Open Alpha\u2019s own exit pair and 18.66% with a SuperTrend(14,4) trail. The aged/deep-base entry filter adds a further +2.60pp of CAGR and takes 6.79pp off the drawdown on top of that.',
  },
  {
    id: 'kc6',
    name: 'KC6',
