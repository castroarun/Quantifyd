#!/usr/bin/env python3
"""Continue InsideDay V2 optimization — runs only remaining configs."""
...
def detect_inside_day_v2(sym, df, i, tp_mult=3.0, sl_mult=2.0, max_hold=5,
    ...
    if outer_range <= 0 or inner_range / outer_range < min_ratio:
        return []
    ...
    if use_ema:
        ...
        long_ok, short_ok = close > ema_val, close < ema_val
    else:
...
def run_shared_pool_v2(all_data, id_params):
    engine = IntradayBacktestEngine(
        ...
        max_positions=20, commission_pct=COMMISSION, slippage_pct=SLIPPAGE,
...
def main():
    # Read already-done configs FIRST
    done = set()
    ...
    configs = [
        ('ID_BASELINE_TP3_MH5', {'tp_mult': 3.0, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': False, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.0}),
        ('ID_TP2_MH5', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': False, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.0}),
        ('ID_TP1.5_MH5', {'tp_mult': 1.5, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': False, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.0}),
        ('ID_TP2_MH3', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 3, 'use_ema': False, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.0}),
        ('ID_TP2_MH5_EMA20', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': True, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.0}),
        ('ID_TP2_MH3_EMA20', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 3, 'use_ema': True, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.0}),
        ('ID_TP3_MH5_EMA20', {'tp_mult': 3.0, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': True, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.0}),
        ('ID_TP2_MH5_VOL', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': False, 'ema_period': 20, 'use_vol': True, 'min_ratio': 0.0}),
        ('ID_TP2_MH5_EMA20_VOL', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': True, 'ema_period': 20, 'use_vol': True, 'min_ratio': 0.0}),
        ('ID_TP2_MH3_EMA20_VOL', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 3, 'use_ema': True, 'ema_period': 20, 'use_vol': True, 'min_ratio': 0.0}),
        ('ID_TP2_MH5_R40', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': False, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.4}),
        ('ID_TP2_MH5_R50', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': False, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.5}),
        ('ID_TP2_MH3_EMA20_VOL_R40', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 3, 'use_ema': True, 'ema_period': 20, 'use_vol': True, 'min_ratio': 0.4}),
        ('ID_TP2_MH5_EMA20_VOL_R40', {'tp_mult': 2.0, 'sl_mult': 2.0, 'max_hold': 5, 'use_ema': True, 'ema_period': 20, 'use_vol': True, 'min_ratio': 0.4}),
        ('ID_TP2_SL1.5_MH5', {'tp_mult': 2.0, 'sl_mult': 1.5, 'max_hold': 5, 'use_ema': False, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.0}),
        ('ID_TP2_SL1.5_MH5_EMA20', {'tp_mult': 2.0, 'sl_mult': 1.5, 'max_hold': 5, 'use_ema': True, 'ema_period': 20, 'use_vol': False, 'min_ratio': 0.0}),
    ]

    remaining = [(l, p) for l, p in configs if l not in done]
    if not remaining:
        print('All 16 configs already completed!', flush=True)
        return

    print(f'\n=== Running {len(remaining)} remaining InsideDay V2 configs ===\n', flush=True)
    ...
    for i, (label, id_params) in enumerate(remaining):
        ...
        print(f' {elapsed:.0f}s | CAGR={result.cagr:.2f}% WR={result.win_rate:.1f}% '
              ...
              f'ID={strat_counts.get("InsideDay", 0)} trades={result.total_trades}', flush=True)
