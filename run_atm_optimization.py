"""
NAS ATM Straddle Optimization Sweep
=====================================
Tests combinations of:
- SL %: 15, 20, 25, 30
- Max re-entries: 0, 1, 2, 3, 5
- Min squeeze bars: 1, 3, 5, 10, 15
- Entry start time: 09:20, 10:00, 11:00
...
OUTPUT_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'optimization_atm_straddle.csv')

FIELDNAMES = [
    'label', 'symbol', 'sl_pct', 'max_reentries', 'min_squeeze_bars',
    'entry_start', 'total_pnl', 'avg_daily_pnl', 'win_rate', 'profit_factor',
...
# Parameter grid
# Phase 1: Focused grid (120 configs, ~30 min)
symbols = ['NIFTY50', 'BANKNIFTY']
sl_pcts = [0.15, 0.20, 0.25, 0.30]
max_reentries_list = [0, 1, 3, 5]
min_squeeze_list = [1, 5, 10]
entry_starts = ['09:20', '10:00']

configs = []
for sym, sl, mre, msq, est in product(symbols, sl_pcts, max_reentries_list, min_squeeze_list, entry_starts):
    label = f'{sym}_SL{int(sl*100)}_RE{mre}_SQ{msq}_E{est.replace(":", "")}'
    if label not in done:
        configs.append((label, sym, sl, mre, msq, est))
...
for i, (label, sym, sl, mre, msq, est) in enumerate(configs, 1):
    t0 = time.time()
