...
BOOKS = {
    "CSL_TIMEB_NIFTY": {**NIFTY_MKT, "lots": 6, "qty": 390, "cfg_from": "lab"},  # 2026-08-17: 6->8 (sec-18b step-2, user)
    "CSL_TIMEB_SENSEX": {**SENSEX_MKT, "lots": 6, "qty": 120, "cfg_from": "lab"},  # 2026-08-18: 6L->8L REAL (notional parity w/ NIFTY TB@8L); suite Wed->paper same night
    ...
    "CSL30F_SENSEX_WED": {**SENSEX_MKT, "lots": 2, "qty": 40, "cfg_from": "fixed",
...
"CSL_TIMEB_NIFTY_MON_AM": {**NIFTY_MKT, "lots": 6, "qty": 390, "cfg_from": "fixed",
...
"NAS_COMB20_FRI": {**NIFTY_MKT, "lots": 2, "qty": 130, "cfg_from": "fixed",
                       "fixed_cfg": {"entry": "09:16", "exit": "15:20", "sl": 30}},  # 2026-08-30: NIFTY is live Mon+Tue only, so COMB's Friday DTE2 cell parks here on PAPER - the Friday record continues without real money. Same pattern as NAS_COMB20_THU.
    "NAS_COMB20_MON": {**NIFTY_MKT, "lots": 2, "qty": 130, "cfg_from": "fixed",
                       "fixed_cfg": {"entry": "09:16", "exit": "15:20", "sl": 30}},  # 2026-08-31 (research/138, Arun): COMB's Monday DTE1 cell moves OFF real money to PAPER, size and stop unchanged. Reason is day allocation, not the parameter - no stop level fixes DTE1 (positive at all six tested, but t only 1.0-1.6 with ~4x DTE3's drawdown, and its replay edge is almost entirely first-half: Rs1,31,845 -> Rs12,960 across halves). Live record was 3 Mondays, all losing, -Rs56,630 at 10-lot equiv. It also stacked correlated size onto the 9:16 suite's STRONGEST day (suite Monday t 2.89) while the held-straddle mechanic earns on Tue/Thu instead. NAS_COMB20 keeps DTE0 (Tuesday) live. Config json seeded DTE1 only. Revisit with the 2026-10-30 ops review.
