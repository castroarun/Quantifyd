...
REVIEWS = [
    ...
    ("research/160 - refresh the point-in-time Screener panel before any reuse",
     "2027-02-01", "PENDING",
     "The panel under research/160_quality_growth_near_ath/results/ (features_pit_monthly.csv.gz + masks/ + masks_study/) was fetched 11-Sep-2026 and stops at 2026-09-01. Screener figures are RESTATED, not as-reported, and the free page depth is about 12 fiscal years, so the panel's earliest usable month walks forward every August as a new FY is filed. Before any future study reuses it, re-run scripts/screener_fetch_full.py (about 2.25 h, resumable) then build_pit_panel.py, build_masks.py and build_study_masks.py. Do NOT reuse a stale panel for a live decision."),
