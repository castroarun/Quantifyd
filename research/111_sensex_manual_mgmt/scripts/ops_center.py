...
REVIEWS = [
    ('Single-name trend-following (SuperTrend / EMA-cross / MST master+child on one or a few stocks) - '
    ...
    ('Long-equity studies - is the PAIRED-ENTRY discipline from research/172 Phase 2 being applied to every stop / trail surface?', '2027-03-13', 'PENDING', 'RAISED 13-Sep-2026 by research/172 Phase 2. Running each exit stack on TWO entries (189-day and 252-day channel) is what exposed the -8% hard stop as an artifact: Calmar 0.408 on one entry and 0.301 on the other, the best and the worst cell in the same grid. A single-entry stop sweep would have shipped it. Also banked there: a hard stop WIDER than the trail is inert by construction and its rows are arithmetic not evidence; and a book-level trailing-drawdown kill that re-arms on a moving average destroyed the book (-2.66% CAGR, -81% DD, 15,990 trades) - do not propose one without this cite. CHECK at the due date alongside the momentum-null review above: the last three stop/trail sweeps in research/INDEX.md each state how many entries (or seeds) the surface was measured on.'),
    ('Long-equity studies - is the MOMENTUM-MATCHED null now in the standard control set, and is the ~4.2pp survivorship haircut being applied to index-CSV screens?', '2027-03-13', 'PENDING', 'RAISED 13-Sep-2026 by research/172. Two reusable results came out of a study that otherwise killed its own idea. (1) A plain random-entry null passed the 52-week channel book; a null drawn from the top HALF of the same universe by 252-day relative strength beat it by 1.8-2.5pp of CAGR on 30 of 30 draws. Any future long-equity study whose signal could be a momentum proxy must run N3/N4 from research/172_52wk_channel_n100/scripts/run172c.py, not just a plain draw. (2) Screening the CURRENT backtest_data/nifty50_official.csv + niftynext50_official.csv over 20 years is worth about +4.2pp of CAGR and +0.24 of Calmar versus a point-in-time liquidity top-100 - subtract it before believing any such result. CHECK at the due date: the last three long-equity studies in research/INDEX.md each state which null they ran and, where they used an index CSV, name the survivorship haircut.'),
    ('IPO Base - first automated IPO exit: placed, filled and booked at the broker price?', '2026-10-15', 'PENDING', 'Deployed 13-Sep-2026 (docs/IPO_BASE_AUTOMATED_EXITS_DAILY_DEPLOY_STATUS.md). At the first IPO exit after 15-Sep: (1) /tmp/ipo_paper.log shows "EXIT DUE" then "EXIT PLACED" at 18:45; (2) the IPO-EXIT SELL is COMPLETE at the next open; (3) backtest_data/ipo_paper_state.json trades has the sale at the broker average with slip_vs_close_pct, the position gone, cash up; (4) the 09:20 equity executor log shows the reconcile ran before sizing; (5) no alert said FAILED TO PLACE. If no IPO exit has happened by the due date, re-date this check.'),
    ('Holiday guard - first live holiday: did every Open Alpha / IPO / cash-park job skip on Mon 14-Sep-2026?', '2026-09-15', 'DONE', 'Deployed 13-Sep (docs/HOLIDAY_GUARD_ALL_BOOK_JOBS_DEPLOY_STATUS.md). CHECK: "skipped - 2026-09-14 is not an NSE trading day" in /tmp/equity_executor.log (09:20), /tmp/cash_park.log (15:10), /tmp/oa_real.log (15:18), /tmp/ipo_paper.log (18:45), /tmp/oa_entry.log (18:50); no new Open Alpha order placed on 14-Sep; no 2026-09-14 point in oa_real_state.json navcurve; journalctl shows "[MP] EOD skipped" for True North at 15:05. DONE 15-Sep-2026: every book job stood down on Mon 14-Sep. Logged skips at 09:20 equity executor, 15:10 and 15:11 cash park, 15:18 OA check, 18:45 IPO cycle, 18:50 OA entry; journalctl shows "[MP] EOD skipped" for True North at 15:05. No order was placed and no value point was written on a closed day.'),
    ("Add config/nse_holidays_2027.json from NSE's official 2027 circular", '2026-12-15', 'PENDING', 'services/trading_calendar.py treats EVERY weekday as a trading day when a year\'s file is missing, so from 1-Jan-2027 every holiday guard, the True North weekly gate and month-end checks, and the NIFTY expiry shift would ignore holidays. NSE publishes the calendar in December. Check future dates against the circular, never a "tentative" list; after each date passes, confirm against market_data.db daily bars (memory nse-holiday-calendar-verify-with-bars). The 2026 file was wrong on 11 dates.'),
    ('True North - did the weekly gate run on Thu 1-Oct (Friday 2-Oct is an NSE holiday)? First live week of the holiday fix', '2026-10-02', 'PENDING', 'Fixed 13-Sep-2026 (docs/TN_WEEKLY_GATE_HOLIDAY_FIX_DEPLOY_STATUS.md): the weekly gate check on the last TRADING day of the week, and a holiday guard on the 15:05 end-of-day job. CHECK on 2-Oct: journalctl -u quantifyd shows the [MP] GATE decision at ~15:05 on Thu 1-Oct and "[MP] EOD skipped" on Fri 2-Oct; mp_state last_weekly reads 2026-10-01. Repeat for Thu 24-Dec.'),
    ('Idle-cash park (CASHIETF) - LIVE TEST on IPO Base with a Rs 10,000 cap, then switch on (needs the 15-Sep rebalance to have landed first)', '2026-09-16', 'PENDING', "Built and deployed SWITCHED OFF on 13-Sep-2026 (docs/CASH_PARK_LIQUID_ETF_IPO_OA_DAILY_DEPLOY_STATUS.md). Arun chose an automated liquid-ETF sweep after Kite Connect was found unable to place mutual-fund orders. Rule: never park money a buy could need before the next release - IPO releases at 09:20 inside services/equity_executor.py before its buy-stops go out, so it keeps a Rs 10,000 buffer; Open Alpha buys by AMO at the open, so it keeps (free slots + 1 swap) x 6.25% x NAV x 1.10 and parks only the excess (nothing today: 5 free slots). A book's cash still includes parked money at cost, so sizing is unchanged; NAV adds the gain; orders use free cash. 21 tests pass, including the executor release path on a fake broker. STEPS: (1) confirm logs/rebalance_mpf_20260915.json says done and IPO cash rose at the 18:45 run; (2) in backtest_data/cash_park.json set enabled.ipo-base true, leave max_order 10000; (3) at the 15:10 run the IPO book parks at most Rs 10,000 - check /tmp/cash_park.log for FILLED, the park block in backtest_data/ipo_paper_state.json (units, cost), static/app/ipo_paper.json nav and park, and `venv/bin/python services/cash_park.py reconcile`; (4) test the release path in market hours: `venv/bin/python services/cash_park.py release --book ipo-base --amount 5000 --arm` - units fall, cash rises by the realised gain only; (5) set max_order to null. Open Alpha stays off until its plan shows a parkable amount. At switch-on, add changelog entries to both rows of frontend/src/data/strategies.ts. Kill: touch backtest_data/executor_kill.flag."),
    ('Capital Desk ONE-OFF REBALANCE 15-Sep-2026 to TN 37.5 / OA 37.5 / IPO 25 - did it run, and did IPO pick up its cash at 18:45?', '2026-09-15', 'DONE', 'Approved by Arun 13-Sep-2026 ("fix IPO, move all Monday"). A detached one-shot, scripts/deferred_rebalance_20260915.sh, wakes 09:45 IST 15-Sep, re-checks the date and a 09:40-14:30 window, and runs scripts/rebalance_mpf_20260915.py --execute. Planned on 13-Sep values: True North -Rs 2,53,615 (Rs 27,578 idle cash + ~Rs 2.26L CASHIETF redeemed), Open Alpha +Rs 41,265, IPO +Rs 2,12,350; recomputed on live values at run time and refused if more than 15% away. It overrides, for this transfer only, the 05-Sep rule that True North is never sold down to rebalance. CHECK: (1) logs/rebalance_mpf_20260915.json status "done" and the .done marker present; (2) True North capital fell by the transfer and its CASHIETF units match the broker; (3) Open Alpha capital and cash rose by its share; (4) after the 18:45 IPO run, backtest_data/ipo_paper_state.json capital equals allocation_targets.json ipo_funded and its cash rose by the IPO share (the FUNDING SYNC fix deployed 13-Sep - before it, a deposit into an already-live IPO book never reached its buying cash); (5) the Capital Desk drift panel reads 37.5 / 37.5 / 25. If the log says "stopped", its reason line names the state it left. RESCHEDULED: moved from Mon 14-Sep-2026, an NSE holiday (Ganesh Chaturthi). Revert of the IPO fix: /tmp/ipo_paper.pre_fundsync.py. DONE 15-Sep-2026 09:46, first attempt. True North -Rs 2,58,396 (214 CASHIETF units redeemed at 1079.55, verified sold before any ledger was cut), Open Alpha +Rs 50,462, IPO +Rs 2,07,934, on a book of Rs 17.54L. The Capital Desk panel reads TN 38.1 / OA 37.5 / IPO 24.5 because it measures CONTRIBUTED CAPITAL while the transfer was computed on CURRENT VALUE - a residual of about Rs 10k per book, not an error. IPO capital reaches its engine at the 18:45 run.'),
