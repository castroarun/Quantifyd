"""Walk-forward — Golden / Death cross."""
...
from run_cross_sweep import (   # type: ignore
    VARIANTS_LONG, VARIANTS_SHORT, signal_fn_long, signal_fn_short, _annotate_crosses,
)
...
def main():
    # Pick top 3 (across both directions) by full-period PF
    rows = []
    ...
    if not pairs:
        pairs = [(VARIANTS_LONG[0], 'LONG'),
                 (VARIANTS_LONG[3], 'LONG'),
                 (VARIANTS_SHORT[0], 'SHORT')]
    ...
    print(f'{"Variant":>30} {"Dir":>5} {"Phase":>5} {"Trades":>7} {"WR%":>6} {"PF":>6} '
    ...
    for cfg, direction in pairs:
        ...
        heartbeat(HB, f'IS {cfg["name"]} {direction}')
        ...
        print(f'{cfg["name"]:>30} {direction:>5} {"OOS":>5} {oos_m["trades"]:>7} {oos_m["win_rate_pct"]:>6.1f} '
