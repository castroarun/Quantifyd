...
Signal: Bollinger Bands BB(20,2) compress *inside* Keltner Channels KC(20,1.5)
for >= MIN_SQZ_BARS bars (volatility compression). When BB exits KC again
("squeeze fires"), and the breakout bar closes beyond BB upper (long) or
lower (short), enter on next bar.

This is a *structural* signal (compression -> expansion) rather than a price-
derived directional signal — different family from MA-cross / VWAP-MR which
both failed in research/13 + 15.

Variants test what filters actually add to the raw squeeze break:
  baseline, +rsi50, +vwap, +volspike, +htf, confluence
"""
...
# Filter variants
VARIANTS = {
    'baseline':   {'rsi50': False, 'vwap': False, 'volspike': False, 'htf': False},
    'rsi50':      {'rsi50': True,  'vwap': False, 'volspike': False, 'htf': False},
    'vwap':       {'rsi50': False, 'vwap': True,  'volspike': False, 'htf': False},
    'volspike':   {'rsi50': False, 'vwap': False, 'volspike': True,  'htf': False},
    'htf':        {'rsi50': False, 'vwap': False, 'volspike': False, 'htf': True},
    'confluence': {'rsi50': True,  'vwap': True,  'volspike': True,  'htf': True},
}
...
def run_stock_variant(df: pd.DataFrame, symbol: str, variant: str, cfg: dict):
    ...
    for day, day_df in df.groupby('day'):
        ...
        for row in day_iter:
            ...
            # Entry signal (squeeze fire on this bar)
            if (active is None and pending is None and not traded_today
                ...
                direction = None
                if bool(row.break_up) and signal_ok(row, 'LONG', cfg):
                    direction = 'LONG'
                elif bool(row.break_down) and signal_ok(row, 'SHORT', cfg):
                    direction = 'SHORT'
                if direction:
