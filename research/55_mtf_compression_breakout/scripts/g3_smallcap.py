"""G3 — SMALLCAP universe (turnover <50cr), the regime where coil->breakout +
volume-spike is supposed to shine. DATA REALITY: smallcap 5-min only exists from
2024-03-18, so this is a ~2-year study (incl. the late-24/early-25 smallcap
correction). Honest about that.

Adds a strong VOLUME-SPIKE arm (vol >= 3x baseline) next to the 1.5x SIGNAL, and
reports per-QUARTER (not per-year) for finer regime view. Keeps the ablation arms
so we still answer: does the compression/breakout/volume entry beat just holding
the uptrending smallcap (BASELINE)?

Incremental/resumable. Run:
  venv/bin/python research/55_.../scripts/g3_smallcap.py
  venv/bin/python research/55_.../scripts/g3_smallcap.py --aggregate-only
"""
...
def universe():
    ...
    sel = set()
    if tc.exists():
        for r in csv.DictReader(tc.open()):
            try:
                if float(r["avg_turnover_cr"]) < TURNOVER_MAX:
                    sel.add(r["symbol"])
            except (ValueError, KeyError):
                pass
    return sorted(sel & have5)
...
def symbol_rows(con, sym):
    ...
    daily = load(con, sym, "day", "2023-06-01", END)   # extra daily for SMA50/CPR warmup
    ...
    for day, g in df15.groupby("day", sort=True):
        ...
        yq = f"{day.year}Q{(day.month-1)//3+1}"
        picks = {}
        if len(g) > 1:
        ...
        for i in range(len(g)-1):
            ...
            ma = float(r["volma"]) if not np.isnan(r["volma"]) else None
            v15 = ma is not None and sl.volume_surge(v, ma, VOL_K)
            v30 = ma is not None and sl.volume_surge(v, ma, VOL_SPIKE_K)
            if "NO_COMPRESSION" not in picks and base and v15:
                picks["NO_COMPRESSION"] = i
            if comp:
                if "NO_VOL" not in picks and base:
                    picks["NO_VOL"] = i
                if "SIGNAL" not in picks and base and v15:
                    picks["SIGNAL"] = i
                if "VOL_SPIKE" not in picks and base and v30:
                    picks["VOL_SPIKE"] = i
        for arm, i in picks.items():
            ...
            for pol, xp in ex.items():
                rows.append({"symbol": sym, "arm": arm, "policy": pol, "yq": yq,
...
def aggregate():
    ...
    print(f"\n=== G3 SMALLCAP — {len(t):,} rows, {t['symbol'].nunique()} symbols, 2024-03..2026-06 ===")
    for pol in POLICIES:
        print(f"\n### {pol}")
        print(f"{'arm':<16} {'n':>7} {'WR%':>6} {'net6R':>8} {'PF6':>6} {'meanGrossR':>11}")
        for arm in ARMS:
            ...
            print(f"{arm:<16} {len(s):>7} {100*(net>0).mean():>6.1f} {net.mean():>8.3f} {pf:>6.2f} {(s['gross_price']/s['R']).mean():>11.3f}")
    print("\n### ALPHA DELTA (arm - TREND_BASELINE) net6R, per policy")
    for pol in POLICIES:
        b = t[(t.policy == pol) & (t.arm == "TREND_BASELINE")]["net6R"].mean()
        line = f"  {pol:<16} BASE {b:+.3f} | "
        for arm in ["SIGNAL", "VOL_SPIKE", "NO_VOL", "NO_COMPRESSION"]:
            a = t[(t.policy == pol) & (t.arm == arm)]["net6R"].mean()
            if pd.notna(a):
                line += f"{arm}:{a-b:+.3f} "
        print(line)
    print("\n### per-QUARTER net6R — SUPERTREND_D (let winners run), SIGNAL vs BASELINE")
    s = t[t.policy == "SUPERTREND_D_10_3"]
    pv = s.pivot_table(index="yq", columns="arm", values="net6R", aggfunc="mean")
    cols = [c for c in ARMS if c in pv.columns]
    print(pv[cols].round(3).to_string())
    print("\n### SIGNAL+VOL_SPIKE trade count per quarter (SUPERTREND_D)")
    print(s[s.arm.isin(["SIGNAL", "VOL_SPIKE"])].groupby(["yq", "arm"]).size().to_string())
...
def run():
    ...
    print(f"smallcap universe={len(uni)} done={len(done)} todo={len(todo)}", flush=True)
    con = sqlite3.connect(str(DB))
    ...
    for n, sym in enumerate(todo, 1):
        ...
        tot += ns
        print(f"[{n}/{len(todo)}] {sym:12s} rows={len(rows):>5} sig={ns:>3} cum={tot}", flush=True)
    f.close(); con.close()
