...
DETAIL = os.path.join(RES, "sx_wed_detail.csv")
SUMMARY = os.path.join(RES, "sx_wed_summary.csv")
...
def thursdays(c):
    ...
    return [d for d in days if date.fromisoformat(d).weekday() == 2]
...
def main():
    ...
    days = thursdays(c)
    log("SENSEX Wednesdays: %d (%s .. %s)" % (len(days), days[0], days[-1]))
    ...
    with open(SUMMARY, "w", newline="") as f:
        ...
        for r in out:
            w.writerow(r)
    log("== SENSEX WEDNESDAY EXIT BAKE-OFF (net Rs per LOT, %d Thursdays) ==" % out[0]["n"])
    for r in out:
