...
def sim(series, pe0, ce0, guard, day_idx, exp, spots, target):
    credit = pe0 + ce0
    ...
    def close_leg(side, fill, d, jj):
        ...
        if live and not rolled:
            ...
            spot_d = spots.get(d)
            if spot_d:
                nk = pick_by_premium(day_idx, d, exp, side, target, spot_d)
                if nk:
    ...
    while j < len(series) - 1:
        ...
        # EOD skew guard: close any heavy leg AT THE CLOSE instead of overnight
        if guard:
    ...
    for entry in mondays:
        ...
        for tn, T in T_GRID.items():
            ...
            for guard in GUARD_GRID:
                net, reason = sim(list(series), pe0, ce0, guard, day_idx, exp, spots, T)
                agg[(tn, guard)].append((net, entry[:4]))
                reasons[(tn, guard)][reason] += 1
    import statistics as st
    log("=== EOD SKEW GUARD (Monday arm, stop 2.0x + PT50 + roll; 2019-2026) ===")
    for tn in T_GRID:
        for guard in GUARD_GRID:
