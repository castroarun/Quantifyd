...
def sim(series, pe0, ce0, action, day_idx, exp, spots, target):
    credit = pe0 + ce0
    ...
    for entry in mondays:
        ...
        for tn, T in T_GRID.items():
            ...
            for action in ACTION_GRID:
                net, reason = sim(list(series), pe0, ce0, action, day_idx, exp, spots, T)
                agg[(tn, action)].append((net, entry[:4]))
                reasons[(tn, action)][reason] += 1
    import statistics as st
    log("=== EOD 1.5x SKEW ACTION: none vs exit-heavy vs recenter (Monday arm; 2019-2026) ===")
    for tn in T_GRID:
        for guard in ACTION_GRID:
