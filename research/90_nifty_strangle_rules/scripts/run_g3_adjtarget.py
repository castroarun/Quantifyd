...
def sim(series, pe0, ce0, adj_frac, day_idx, exp, spots, target):
    action = 'rc1'
    rollmode = 'restrangle'
    adj = max(5.0, adj_frac * target)
    credit = pe0 + ce0
        ...
        if hit:
            if rollmode == 'restrangle' and not rolled:
                ...
                for s2 in list(live):
                    ...
                    exit_prem += bars[s2][2]
                    del live[s2]
                fresh = sell_fresh(day_idx, d, exp, spots, adj)
                if not fresh:
...
def main():
    ...
    for entry in mondays:
        ...
        for tn, T in T_GRID.items():
            ...
            for adj_frac in ADJ_FRACS:
                net, reason = sim(list(series), pe0, ce0, adj_frac, day_idx, exp, spots, T)
                agg[(tn, adj_frac)].append((net, entry[:4]))
                reasons[(tn, adj_frac)][reason] += 1
    import statistics as st
    log("=== ADJUSTMENT TARGET (restrangle+recenter) as fraction of entry T, v1.3 base (Monday arm; 2019-2026) ===")
    for tn in T_GRID:
        for guard in ADJ_FRACS:
