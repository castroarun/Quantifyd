"""SENSEX-adapted parametrized true-replay engine for the 6 ATM NAS systems on the recorded NIFTY
...
LOT = 20; LOTS = 2; QTY = LOT * LOTS      # normalized 2 lots for cross-system comparability
...
days = [r[0] for r in oc.execute(
    "SELECT DISTINCT substr(snapshot_time,1,10) FROM option_chain WHERE symbol='SENSEX' ORDER BY 1")]
...
def load_day(day):
    ...
    df = pd.read_sql_query(
        ...
        "FROM option_chain WHERE symbol='SENSEX' AND substr(snapshot_time,1,10)=? AND ltp IS NOT NULL",
        oc, params=[day])
    if df.empty:
        return None
...
def sim_loaded(bundle, entry_mode, mgmt, stop_mode="premium", move_pct=0.004,
               ...
               move_stop_reenter=False, reenter_cooldown_min=0, record_mtm=False):
        ...
        if record_mtm:
            ...
            for _l in legs:
                if _l["open"]:
                    _p = prem(_l["tsym"], t)
                    if _p is not None:
                        _un += (_l["entry"] - _p) * _l["qty"] - BROK_PER_LEG
            mtm.append((str(t), running_realized + _un))
            ...
            if stop_mode == "premium" and p >= lg["sl"]:
                if mgmt == "SL_ST":
                    ...
                    if strike_gate:
                        ...
                        if _cur_k is not None and round(_sp / 100) * 100 == int(_cur_k):
    ...
    if record_mtm:
        mtm.append((str(last_t), running_realized))
        return realized, mtm
    return realized
