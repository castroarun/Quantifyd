"""Phase 06 — same drawdown-control + OOS/post-tax pipeline, SMALL-cap core.
...
BAND, L, N = "small", None, 15          # L=None -> blended 6m+12m RS
...
def main():
    ...
    close, tv = rs2.load()
    print(f"  small core = small_blend_6m12m_N15, benchmark={BENCH}",
          flush=True)
    FULL = ("2014-01-01", "2026-12-31")

    b0, _ = backtest(close, tv, None, None, False, *FULL)
    M0 = m(b0)
    print(f"\nSMALL RS-ALONE baseline (small_blend_N15): "
          f"CAGR={M0['cagr']:.1f}% Sh={M0['sh']:.2f} DD={M0['dd']:.1f}% "
          f"Cal={M0['cal']:.2f}", flush=True)

    print("\n=== SMALL: drawdown-control overlay sweep ===", flush=True)
    rows = [dict(config="SMALL_baseline", q=None, dd=None, regime=False,
    ...
    df.to_csv(RES / "phase06_smallcap_overlay.csv", index=False)

    if best is None:
        print("\nNo small overlay holds CAGR>=30% — small stays inferior "
    ...
    print("\n=== SMALL champion — OOS sub-period ===", flush=True)
    ...
    print("\n=== SMALL champion — post-tax (STCG) ===", flush=True)
    ...
    print("\n=== MID vs SMALL (gated, post-tax @20% STCG) ===")
