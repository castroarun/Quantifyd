...
# Trimmed universe: top 100 by recent avg daily turnover (mirrors run_volbo_500.py).
# Threshold ~Rs 172 Cr/day. Cuts CCRB wall from ~22h to ~7h.
TOP_100_BY_TURNOVER = [
...
def _load_universe() -> list[str]:
    """Phase G (2026-05-07): expanded to all stocks at Rs 50+ Cr/day avg turnover.

    Reads research/34/results/top_by_turnover.csv. Resumable skip-set means
    the 100 stocks already swept in Phase C are skipped immediately and only
    the 118 new names get processed.
    """
    import csv as _csv
    import sqlite3
...
print(f"[run_ccrb_500] Universe loaded: {len(ALL_STOCKS)} stocks "
...
LIQUIDITY_MIN_TURNOVER = 5_00_00_000
...
def build_ctx_variants():
    """Trimmed grid for research/34 (Nifty 500): drop ctxN and ctxW_AND_N.

    Per research/32: ctxN alone fires <0.32 stocks/day in liquid universe (dead).
    Per research/31: ctxW_AND_N fires zero cells in any stock (always empty).

    Keeping: W (3 variants) + W_OR_N (9 variants) = 12 ctx variants.
    Halves cell count vs research/31 (24 -> 12) without losing real signals.
    """
    out = []
    # W only — 3 variants
    for w in YESTERDAY_WIDES:
        out.append(("W", w, YESTERDAY_NARROW_RANGES[1]))   # narrow filler (unused)
    # W_OR_N — 9 variants (3x3)
    for w in YESTERDAY_WIDES:
...
CTX_VARIANTS = build_ctx_variants()       # length 12 (was 24 in research/31)
                ...
                for vmode in VOL_MODES:
                    for direction in DIRECTIONS:
                        ...
                        for s in ccrb_signals(
                            ...
                            # 20-day median price < Rs 50 OR turnover < Rs 5cr
