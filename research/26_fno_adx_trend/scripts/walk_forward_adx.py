"""Walk-forward — ADX trend strength."""
...
from run_adx_sweep import (   # type: ignore
    VARIANTS_LONG, VARIANTS_SHORT, signal_fn_long, signal_fn_short, _annotate_adx_cross,
)
...
def main():
    ...
    print(f'{"Variant":>22} {"Dir":>5} {"Phase":>5} {"Trades":>7} {"WR%":>6} {"PF":>6} '
    ...
    for cfg, direction in pairs:
        ...
        print(f'{cfg["name"]:>22} {direction:>5} {"IS":>5} {is_m["trades"]:>7} {is_m["win_rate_pct"]:>6.1f} '
        ...
        print(f'{cfg["name"]:>22} {direction:>5} {"OOS":>5} {oos_m["trades"]:>7} {oos_m["win_rate_pct"]:>6.1f} '
