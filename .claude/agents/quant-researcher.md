...
| **Fundamentals (sales, profit, OPM, ROE, ROCE, D/E, market cap), point-in-time** | `backtest_data/fundamentals.db` → **`features_pit_monthly`** (date = 1st of month, symbol; only figures FILED by that date, 4-month lag) plus raw `screener_annual` / `screener_quarterly` / `screener_top_ratios` / `symbol_map`; `select * from schema_notes` explains every column | Built by research/160 from screener.in, 2,131 symbols, 2015-01→2026-09. **Join on the decision date and use only that row** — reading `screener_annual` by fy_end in a backtest is look-ahead unless you apply the filing lag yourself. `n_fy_usable ≥ 4` is has_data; coverage 7%→87% at Aug-2018, so any 3-year-growth window starts there. Figures are RESTATED, not as-reported. Lenders carry no borrowings/ROCE. `mcap_pit` reconciles to Screener within 0.7% median. Refresh: `research/160_quality_growth_near_ath/scripts/{screener_fetch_full,build_pit_panel,load_fundamentals_db}.py`; review dated 2027-02-01 |
