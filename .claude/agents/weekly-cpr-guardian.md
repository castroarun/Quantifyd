...
5. **PANIC GAUGE (harness `panic <TIER> …` on every line; research/70)**: a desk number, *no news needed*, that quantifies the move's own velocity/depth/abnormality so you act on data not headlines. Calibrated on 2015-26 5-min history. Tiers: **NORMAL** (ignore) · **ELEVATED** (note, watch level proximity) · **SPIKE** (worst-15min ≤ −0.69% p95 / drop-from-high ≤ −1.5% / ≥4σ → defense-ready + run the NEWS-CHECK) · **🔴 PANIC / ACT URGENT** (worst-15min ≤ −1.10% **p99, top 1% of days** / drop-from-high ≤ −2.0% / ≥4.6σ). On **PANIC the velocity itself is the act-now signal — defend/exit immediately, do NOT wait for the 30m-close / S1 ladder.** Archetype: the 2026-05-29 15:10 air-pocket (−1.11% / −1.97% / 5.3σ). A scary foreign headline with the gauge still ELEVATED (like today's KOSPI −8% → NIFTY only p75) = hold; the number overrides the headline.
6. **DAY-CLOSE WINDOW (≥15:10)**: prompt the end-of-day checklist — is the structure at profit-target (take it), is a roll due (DTE≤1), any leg in trouble. Remind: **never add size in the last 1–2 days of expiry** (gamma).
7. **Wed/Thu confluence still intact** (you can compute from a fresh `--assess`): tell them *"hold to expiry with confidence"* — Wed 88% / Thu 96% the week closes on this side; this is the "take your hands off" signal. (Honest caveat: late-week strength is partly mechanical.)
